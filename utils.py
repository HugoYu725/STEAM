...
def load_data1(file_path):
    ...
    data = np.loadtxt(file_path, dtype=[('src', int), ('dst', int), ('time', int)], comments='%', usecols=(0, 1, 3)) # 读取0，1，3列的数据作为src、dst、time，%为注释符号 实际上就是交互信息
    data.sort(order = 'time')   # 数据按照时间戳排序
    src_node = data['src']
    dst_node = data['dst']
    times = data['time']
    edges = np.vstack((src_node, dst_node)).T   # 堆叠节点对就是边信息
    orgin_node_ids = np.unique(edges) # re tag the node_id  节点id-1899
    n = len(orgin_node_ids) # 节点总数
    node2id = {orgin_node_ids[i]:i for i in range(n)}   # n对应n-1的字典
    src_node = [node2id[node] for node in src_node] # 源节点id
    dst_node = [node2id[node] for node in dst_node] # 目标节点id  都是-1的
    edges = np.vstack((src_node, dst_node)).T   # 堆叠 使得节点下标从0开始而不是1
...
def generate_snapshots(edges, n,times):
    snap_list = []
    edges_num = len(edges)
    snap_edge_time=[]
    ...
    for i in range(n-1):
        ...
        snap_edge_time.append(max(times[i*snap_edges_num:(i+1)*snap_edges_num])-times[i*snap_edges_num:(i+1)*snap_edges_num])
    snap_list.append(edges[(i+1)*snap_edges_num:])
    snap_edge_time.append(max(times[(i+1)*snap_edges_num:])-times[(i+1)*snap_edges_num:])
    for i in range(10):
        snap_edge_time[i]=(snap_edge_time[i]+1)/(snap_edge_time[i][0]+1.0000001)
    return snap_list,snap_edge_time
...
def sparse_mx_to_torch_sparse_tensor(sparse_mx):
    ...
    sparse_mx = sparse_mx.tocoo().astype(np.float32)    # 转csr-->coo
    indices = torch.from_numpy(
        np.vstack((sparse_mx.row, sparse_mx.col)).astype(np.int64)) # 堆叠行列
    values = torch.from_numpy(sparse_mx.data)   # ??? # 边权 numpy数组转张量
